...
def read_route_binary_definitions_from_rust_project(project_root: Path) -> Iterator[RustBinaryDefinition]:
    ...
    with open(manifest_path) as manifest:
        binary_definitions = ""
        # TODO: Nicer error handling rather than just StopIteration if the boundaries
        # can't be found.
    ...
    # TODO: Nicer error handling than just KeyError if the declarations are not in
    # the shape we expect.
    return (RustBinaryDefinition(
        name = definition["name"],
        path = project_root.joinpath(definition["path"]),
    ) for definition in tomllib.loads(binary_definitions)["bin"])
